...
data.badstart %>% group_by(Treatment) %>% count() # 24 fragments: 18 Control and 6 MAT
...
# Exclude missing fragments and fragments with heavily reduced condition due to predation
...
EV_TreatmentxSpecies <- ggplot(P.data.sum, aes(x = Species, fill = Treatment, y = EV.increase))+
  ...
  scale_y_continuous(expand = c(0, 0), breaks = c(0, 1000, 2000, 3000), limits = c(0, 3600))+
